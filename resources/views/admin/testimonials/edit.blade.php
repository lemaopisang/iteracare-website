...
<div class="space-y-6">
    ...
    <div class="bg-white shadow sm:rounded-lg">
        <form method="POST" action="{{ route('admin.testimonials.update', $testimonial) }}" class="space-y-6 p-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="4" required
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old("content", $testimonial->content) }}</textarea>
                @error("content")
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="url" name="video_url" id="video_url" value="{{ old("video_url", $testimonial->video_url) }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                @error("video_url")
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                @if($testimonial->video_file)
                    <p class="mt-2 text-sm text-gray-600">Video saat ini: <a href="{{ asset('storage/' . $testimonial->video_file) }}" target="_blank" class="text-blue-600 underline">Lihat Video</a></p>
                @endif
                @error('video_file')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="user_id" id="user_id" value="{{ old("user_id", $testimonial->user_id) }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
